...
public class neuerPostenScreenController {
	...
	public void initialize() throws SQLException {
		String [] k = model.return_Kategorien();
		Arrays.sort(k);
...
/* beim auswählen eines wertes in der Kategorie combobox wird die Subkategorie combobox aktiviert 
 * und die jeweiligen Daten sortiert eingetragen
 */
	@FXML
	void cbA_kategorieInput_neuerPosten(ActionEvent event) throws SQLException {
		String [] s = model.return_Subkategorien(kategoerieInput_neuerPosten.getValue());
		Arrays.sort(s);
	...
	void bA_übernehmen(ActionEvent event) throws SQLException {
		try {
			...
			if (p.isEmpty() || a.isEmpty() || pr.isEmpty() || kat == null
				...
				/* prüft ob in den feldern etwas steht, wenn nicht, dann wird ein rotes "*" bei diesem feld angezeigt */
				if (p.isEmpty()) {
				...
				if (kat == null) {
					...
					kategorie_labelred_neuerPosten.setText("");
				if (s == null) {
					subkategorie_labelred_neuerPosten.setTextFill(Color.web("red"));
			...
			/* prüft ob die Anzahl eine Integer Zahl ist */
				try {
					...
					übernehmen_label_neuerPosten.setText("Die Anzahlangabe ist falsch");
					anzahl_labelred_neuerPosten.setText("*");
					return;
				...
				/* gibt jedem Wert in der Dauerauftrag combobox eine Integer Zahl zu (Anzahl der 
				 * Monate)
				 */
					try {
						switch (dauer) {
				...
				/* prüft ob das Budget noch ausreicht. Wenn es ausreicht, wird der Posten übernommen
				...
				init();
					...
					übernehmen_label_neuerPosten.setTextFill(Color.web("red"));
					übernehmen_label_neuerPosten.setText("Ihr Budget reicht nicht aus");
				}	
		}
